...
<a href="index.php"><input name="" type="button" class="back_btn" value="<<Back"/></a>
					...
					<div class="view_client_row">
						...
						<div class="client_right">
							<label>Date</label>
							<label class="filled_label"><?php $datee=$row['p_date']; list($a,$b)=explode(" ",$datee); echo $a; ?> </label>
						</div>
					</div>
					<div class="view_client_row">
						<div class="client_left">
					...
					<div class="view_client_row">
						<div class="client_left">
							...
							<label class="filled_label"><?php echo $d= $row['p_d_o_b']; ?></label>
						</div>
						<div class="client_right">
				...
				<div class="client_box_bg">
					...
					<?php
						/*$temp_temp_profile = mysql_query("SELECT * FROM `documents_messages` WHERE `main_user_id`='$attorneys_id'") or die(mysql_error());
					...
					?>
						<div class="client_row_content">
							<div class="client_span_1"><?php echo $form_i = $f_temp_profile->form_id; ?></div>
							<div class="client_span_2">
								<?php 
									echo $name = $temp_profile->GetInfoPlantiffInformation($form_i,"plantiff_name").'&nbsp;';
								?>
							</div>
							<div class="client_span_3"><?php echo $email_address = $temp_profile->GetInfoPlantiffInformation($form_i,"email_id") ?></div>
							<div class="client_span_4">
								<?php 
									$temp_state = $temp_profile->GetInfoPlantiffInformation($form_i,"p_state");
									if($temp_state=="")
									{
							...
							<div class="client_span_5">
								<?php 
									...
									echo $dattime = date('d-M-Y',strtotime($date));
								?>
								</div>
							<div class="client_span_6">
								<a href="other-documents.php?fid=<?php echo $f_idss = $f_temp_profile->form_id; ?>&case_id=<?php echo $temp_profile->GetCaseTypeByFormID("case_type",$f_idss); ?>&id=<?php echo $f_temp_profile->user_id; ?>&action=upload">Upload Documents</a>
							</div>
						</div>
					<?php
						}
					...
					<div class="anesth_box_bg">
						...
						?>
							<div class="anesth_row_content">
								...
								<div class="anesth_span_4"><?php $date_upload=$uploads->upload_date;?> <?php echo date ('m-d-Y',strtotime($date_upload)); ?></div>
