	...
	<?php }?>
				...
				<?php
					if(isset($_POST['up_user_documents']))
					{
						...
						('$_REQUEST[fid]','$_REQUEST[id]','".$att_ids."','".$related_to."','$_REQUEST[cid]','".$document_name."','".$newfilename."','".$message."','".$current_date."')";
						...
						$notification = $sql = mysql_query("INSERT INTO `notifications` (`user_id`,`form_id`,`main_id`,`message`) values ('$_REQUEST[id]','$_REQUEST[fid]','','$messaged')") or die(mysql_error());
						
						$inserMessage = mysql_query("INSERT INTO `message_sent` (`form_id`,`user_id`,`sent_by`,`main_user_id`,`message`,`date_message`) VALUES ('$_REQUEST[fid]','$_REQUEST[id]','$att_ids','','$messaged',now())") or die(mysql_error());
						
						if($move)
						{
							echo $filename." Uploaded Successfully";
							header("Refresh:2;url=".$sitepath."/case-manager/full-details.php?fid=$_REQUEST[fid]&uid=$_REQUEST[id]&cid=$_REQUEST[cid]");
						}
					}
				?>	
			</div>
