 ...
 * */
 ...
 class Registration
 {
	 public function UserRegister($user_name,$password,$user_email,$registration_datetime,$registration_ip)
	 {
			...
			$sql=mysql_query("SELECT `user_id` FROM `admin_reg` WHERE `user_name`='$user_name' or `user_email`='$user_email'") or die(mysql_error());
			
			if(mysql_num_rows($sql)==0)
			{
				
				$result=mysql_query("INSERT INTO `admin_reg` (`user_name`,`user_password_hash`,`user_email`,`user_registration_datetime`,`user_registration_ip`) 
