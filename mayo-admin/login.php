...
$login = mysql_query("SELECT * FROM `members` WHERE user_name='$username' && `password`='$password' && `activated`='1' && `designation`=8") or die(mysql_error());
$logintemp = mysql_fetch_array($login);
...
if ($loginok==TRUE) //if it is the same password, script will continue.
{
	if ($remember == "yes") //if the Remember me is checked, it will create a cookie.
	{
		...
		$_SESSION['username']=$username;
		echo $_SESSION['designation']=$logintemp['designation'];
	...
	{
		$_SESSION['username']=$username;
		$_SESSION['designation']=$logintemp['designation'];
