...
<form name="userinfo" method="post" action="" id="regform">
	<ul>
		<li>
			...
			<select name="" class="sel_reg_form" required>
			<option value="">...Select...</option>
				<?php 
					$sql = mysql_query("select * from `designation` where id!=5 || id!=8") or die(mysql_error());
					while($row = mysql_fetch_object($sql))
					{
					echo "<option value='".$row->id."'>".$row->sub_designation_name."</option>";
					}
				?>
			</select>
