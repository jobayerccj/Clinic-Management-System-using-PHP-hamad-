	...
	<div class="container">
		<div class="form_section_content">
			...
			if(isset($_POST['add-role']))
			{
				...
				$data      = array('cpt_code'=>$_POST['cpt'],'description'=>$_POST['description']);
				$tablename = "cpt_codes";
		...
		?>
				...
				<?php
					if(isset($_POST['update_cpt']))
					{
						...
						$update_temp = mysql_query("UPDATE `cpt_codes` SET `cpt_code`='$cpt',`description`='$desc' WHERE `id`='$_REQUEST[cptid]'") or die(mysql_error());
						if($update_temp)
						{
			...
			<div class="view_log_details">
				<div class="log_heading">
					<div class="name_1">CPT Code</div>
					<div class="desc_1">Description</div>
					<div class="edit_1">Edit</div>
					<div class="delete_1">Delete</div>
				</div>
				<?php 
					$query =mysql_query("SELECT * FROM `cpt_codes` order by id desc") or die(mysql_error());
					$ij=1;
					while ($row = mysql_fetch_array($query)) 
					{
				?>
					<div class="log_row">
						<div class="name_1"><?php  echo $row['cpt_code']; ?></div>
						<div class="desc_1"><?php  echo $row['description'];?></div>
						<div class="edit_1"><a href="insert-cpt-code.php?cptid=<?php echo $row['id']; ?>&action=edit">Edit</a></div>
						<div class="delete_1">
							<a class="confirmation<?php echo $row['id']; ?>" href="insert-cpt-code.php?cptid=<?php echo $row['id']; ?>&action=delete">Delete</a>
						</div>
					</div>
					<script type="text/javascript">
						$('.confirmation<?php echo $row['id']; ?>').on('click', function () {
