...
function getInfo(usersId) {		
	...
	function getValues(addcptid) 
	{	
		var strURL="find-values.php?ctp_code="+addcptid;
		var req = getXMLHTTP();
		if (req) 
		{
			req.onreadystatechange = function() 
...
<h1>Uploaded Documents</h1>
			...
			<div class="dashbord_client">
				...
				<div class="client_box_bg">
					<div class="dashboard_row_heading">
						<div class="dashboard_span_h1">Case Type</div>
					...
					?>
								<div class="client_row_content">
									...
									<div class="dashboard_span_h6">
										<a id="deletestaff" href="delete-c-staff.php?hid=<?=$data->hire_id?>&fid=<?=$_REQUEST['id']?>&uid=<?=$_REQUEST['uid']?>">Delete</a>	
									</div>
								</div>
						<?php 
							}
						...
						?>
								<div class="update_status_row">
									...
									<div class="update_status_span_2">
										<?php 
											echo 	$a_id = $getstatus->main_user_id;
		...
		<div class="tabbertab">
			...
			<div class="dashbord_client">
				<div class="update_status_top">
					<h1>Message</h1>
						<input type="checkbox" id="squaredFour" class="selectall" name="msgid" value="selectall">

						<script type="text/javascript">
							/*
								Script to check all the messages
							*/
						...
						<form name="update_status" method="post" action="">
						<?php
							$sql = mysql_query("SELECT * FROM `hire_staff` WHERE `form_id`='$_REQUEST[id]' && `user_id`='$_REQUEST[uid]'") or die(mysql_error());
							while($hires = mysql_fetch_object($sql))
							{
								$hire_staff  = $hires->hire_id;
								$first_name  = $getinformation->GetObjectById($hire_staff,"first_name");
								$last_name   = $getinformation->GetObjectById($hire_staff,"last_name");
								$designation = $getinformation->GetObjectById($hire_staff,'designation');
								$desg_name   = $getinformation->GetDesgBydesId($designation)."<br/>";
							?>
									<input type="checkbox" name="msgid[]" id="squaredFour" value="<?php echo $hire_staff; ?>" />
									<label>
						...
						?>
						<textarea name="send_message" placeholder="Send Message"></textarea>
						<input type="submit" name="send_mess_ad" id="" value="Submit"/>
					</form>
					<?php
						if(isset($_POST['send_mess_ad']))
						{
							$msgs_ids = $_POST['msgid'];
							$message  = mysql_real_escape_string($_POST['send_message']);
							foreach($msgs_ids as $sent_to_id)
							{
								$temp_id[] = $sent_to_id;
							}
							echo $temp_ids  = implode(',',$temp_id);
							$insert_mes = mysql_query("INSERT INTO `message_sent` (`form_id`,`user_id`,`sent_by`,`main_user_id`,`message`,`date_message`) VALUES ('$_REQUEST[id]','$_REQUEST[uid]','$admin_id','$temp_ids','$message',now())") or die(mysql_error());
							if($insert_mes)
							{
								echo "<div class='e_message'>Message has been successfully sent to following Users.</div>";
							}
							else
							{
								echo "<div class='e_message'>Something Going Wrong. Please try again later.</div>";
							}
						}
					?>
				</div>
		...
		<div class="tabbertab">
			...
			<div class="dashbord_client">
				<div class="billing_box_bg">
					...
					<?php
						if(isset($_POST['add_cpt']))
						{
							...
							`date_bill`) VALUES ('$_REQUEST[id]','$_REQUEST[uid]','$ctp_code','$description','$phy_loc','$physician','$facility','$other',now())")  or die(mysql_error());
							if($query_bill)
							{
				...
				<div class="anesth_box_bg">
							...
							<?php
								$query = mysql_query("SELECT * , SUM( physician + facility + other_total+loc) AS total FROM `billing_info`WHERE form_id = $_REQUEST[id] && `user_id`='$_REQUEST[uid]'  GROUP BY cpt_code") 
								or die(mysql_error());
							...
							?>
									<div class="anesth_row_content">
										...
										<div class="anesth_span_9">
											<a href="update-billing.php?id=<?=$_REQUEST['id']?>&uid=<?=$_REQUEST['uid']?>&billing_id=<?=$getapp->billing_id?>&action=update">Update</a>
										</div>
									</div>
								<?php
								 }
							...
							<div class="anesth_box_bg">
								<?php 
									$sql = mysql_query("SELECT SUM(physician) AS tPhy, SUM(facility) AS tfac, SUM(loc) as tloc, SUM(other_total) AS tother, (SUM(physician) + SUM(facility) + SUM(other_total)) AS total FROM `billing_info` WHERE form_id = '$_REQUEST[id]' && user_id = '$_REQUEST[uid]'") or die(mysql_error()); 
								...
								<div class="anesth_row_heading">
									...
									<div class="anesth_span_9">
										<a href="forward-bill.php?id=<?=$_REQUEST['id']?>&uid=<?=$_REQUEST['uid']?>&action=update">Forward</a>
									</div>
								</div>
								<div class="anesth_row_content">
									<div class="anesth_span_1">
				...
				<div class="tabbertab">
					...
					<div class="dashbord_client">
						<div class="billing_box_bg">
							...
							<form name="form1" method="post" action="">
								<div class="close_case"><strong>Close Case :</strong> <input class="confirmation" type="submit" name="case_close" value="Close Case"></div>
								<div class="close_case"><strong>Delete Case:</strong> <input class="confirmation" type="submit" name="delete_case" value="Delete Case"> </div>
							</form>
