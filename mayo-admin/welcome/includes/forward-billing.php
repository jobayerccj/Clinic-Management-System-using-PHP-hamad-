...
<div class="billing_box_bg">
					...
					if(mysql_num_rows($qry)>0)
					{
						...
						echo '<a class="back_btn" style="margin-top:55px;" title="If billing is already forwarded and you want to add more CPT Codes then delete forwarded billing from here. And then add the CPT codes and again forward the billing to particular Underwriter." href="#" id="del_forwarded_billing" dfbid="'.$data->b_f_id.'">Delete Forwarded Billing</a></div>';
				...
				?>
					<form name="forwardbill_doctor" id="otho_groupss" method="post" action="">
						<select name="doctor" onchange="" required>
...
AND a.designation =3 && b.form_id='$_REQUEST[fid]' && b.user_id='$_REQUEST[uid]'") or die(mysql_error());
							...
							if(isset($_POST['reply_doctor']))
								{
									...
									$sql     = mysql_query("INSERT INTO `message_doctor_billing` (`main_id`,`form_id`,`user_id`,`sent_by`,`message`,`date_time`) VALUES ('$doc_id','$form_id','$user_id','$admin_id','',now())") or die(mysql_error()); 
				...
				<?php
					...
					if(isset($_POST['forward_bills']))
					{
						...
						//insert this in the messages
						
						
						
						/*Query used to insert the messages*/
						$temp_send   = mysql_query("INSERT INTO `message_doctor_billing` (`main_id`,`form_id`,`user_id`,`sent_by`,`message`,`date_time`) VALUES ('$doctor','$_REQUEST[fid]','$_REQUEST[uid]','$admin_id','',now())") or die(mysql_error());
						$dattime = date('d-M-Y h:i:s a');
						...
						*/
						...
						$messToUnder = "Approval of Billing by Underwriter and Mayo for Client ".$plan_name;
						//insert in to messages.
						$message     = mysql_query("INSERT INTO `message_sent` (`form_id`,`user_id`,`sent_by`,`main_user_id`,`message`,`date_message`) VALUES ('$form_id','$user_id','$admin_id','$doctor','$messToUnder',now())") or die(mysql_error());
						...
						*/
						...
						$message="";
						$message .= '<html><body>';
						$message .= '<img src="https://mayosurgical.com/images/logo.png" alt="Surgery Approval Notification  (Approval of Billing by Underwriter and Mayo" />';
						...
						$temp_getinfo  = mysql_query("SELECT a.id AS hire_id, b. * FROM members AS a, hire_staff AS b WHERE a.id = b.hire_id AND a.designation =3 && b.form_id='$_REQUEST[fid]' && b.user_id='$_REQUEST[uid]'") or die(mysql_error());
						$data_hhhire = mysql_fetch_object($temp_getinfo);
						$hireDoctorId = $data_hhhire->hire_id;
						$blingToD = mysql_query("SELECT a.*,b.* FROM `billing_info` as a, admin_billing as b WHERE a.`form_id` = '$_REQUEST[fid]' && a.`user_id`= '$_REQUEST[uid]' && a.cpt_code=b.cpt_code and b.id='$hireDoctorId'") or die(mysql_error());
						...
						$message .='<tr><td>****All Surgical Referrals should include Mayo Surgical, LLC on Invoice as the Guarantor on the account and ALL BILLS forwarded to Mayo Surgical LLC, not the patient****</td></tr>';
						...
						if($temp_send)
						{
							echo "<div class='thank_message'>Notice is forwarded to <b>".$firstname."&nbsp".$lastname. "</b></div>";
