...
<div style="margin:0px auto; width:75%;">
	<form style="margin:auto;width:75%;" name="register-sub-group" method="post" action="">
		...
		<div class="attorney_row_form">
			...
			<?php
				$sql = mysql_query("SELECT * FROM `sub_designation` where `designation_id`=4") or die(mysql_error());
				while($row = mysql_fetch_object($sql))
				{
			?>
				<option value="<?=$row->designation_id?>"<?php if($subDes == $row->id){echo "select='selected'";} ?>><?=$row->sub_designation_name?></option>
			<?php
				}
			?>
			</select>
				...
				<div class="attorney_row_content">
					...
					<div class="attorney_client_6">
						<ul>
							...
							<!--<li><a href="?action=assign&id=<?php //echo $row['id']?>" class="attorney_assign" title="Assign Work">Assign Work</a></li>-->
						</ul>
					</div>
				</div>
				<?php
