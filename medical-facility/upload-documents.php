			...
			<div class="dashbord_client">
				...
				<div class="attorney_box_bg">
					<div class="attorney_row_heading">
						...
						<div class="dr_new_client_3">Document Description</div>
						<div class="dr_new_client_4">Action</div>
					</div>
					<?php
					$documents = mysql_query("select a.id,b.* from members as a,upload_documents as b where a.id=b.attorney_id and a.designation!=6 and a.designation!=3 and `user_id` = '$_REQUEST[uid]' && `form_id` = '$_REQUEST[fid]' && `related_to` !=  'Estimate of Doctor and Facilities Charges'") or die(mysql_error());
					while($listdocuments  = mysql_fetch_array($documents))
					{
				?>
					<div class="attorney_row_content">
						<div class="dr_new_client_1"><?php $getdate = $listdocuments['upload_date']; echo date('m-d-Y',strtotime($getdate)); ?></div>
						<div class="dr_new_client_2"><?php echo $listdocuments['related_to']; ?></div>
						<div class="dr_new_client_3"><?php echo $listdocuments['message']; ?></div>
						<div class="dr_new_client_4">
							<a href="<?php echo $sitepath ?>/uploads/<?php echo $listdocuments['upload_document_path']; ?>" target="_blank">Download</a>
						</div>
					</div>
				<?php 
					} 
